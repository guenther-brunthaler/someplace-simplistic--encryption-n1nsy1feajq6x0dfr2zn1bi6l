#define VERSTR_1 "Version 2020.359"
...
static char help[]= { /* Formatted as 66 output columns. */
   ...
   "Supported options:\n"
   "\n"
   "-E: Selects raw encryption mode. Minimally slower then decryption.\n"
   "-D: Selects raw decryption mode. Also recommended for hashing.\n"
   "\n"
   "-M <mac_key>: Use <mac_key> as the pathname of a second binary key\n"
   "file, providing a MAC key. May also refer to a special file such\n"
   "as '/dev/fd/9'. In encryption mode, this will calculate and append\n"
   "a 32-octet MAC of the encrypted data after the encrypted data. In\n"
   "decryption mode, it will expect the last 32 octets of the input\n"
   "stream to represent a MAC and will verify that it matches the\n"
   "encrypted input data preceding it.\n"
   "\n"
