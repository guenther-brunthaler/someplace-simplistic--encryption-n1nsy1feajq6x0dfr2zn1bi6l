#define VERSTR_1 "Version 2020.330"
...
static char help[]= { /* Formatted as 66 output columns. */
   ...
   "data ciphertext read from standard input and writes the binary\n"
   "result to standard output. Note that actual human-recognizable\n"
   "text data is a subset of binary data and will therefore also work\n"
   "as plaintext.\n"
   "\n"
   ...
   "\n"
   "When used on their own, ARCFOUR-drop3072 has a few moderate\n"
   ...
   "be reused for different messages when using treyfer-ofb! Use at\n"
   "least a different IV for every message using the same key. Or\n"
   "even better, derive both from a long-term key/password and a\n"
   "unique per-message salt (a non-secret nonce).\n"
   "\n"
   ...
   "another copy of the salt as the first 32 octets as the encrypted\n"
   "file, allowing the salt to be retrieved from there for\n"
   "decryption.\n"
   "\n"
